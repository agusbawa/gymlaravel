...
class extendsController extends Controller
{
    public function member(Request $request)
    {
        ...
        $members = Member::get();
        return view('dashboard.report.extends.member',compact('nama_gym','gym','zona','tertentugym','tertentuzona'))->with('members',$members)
    ...
    public function searchmember(Request $request)
    {
        ...
        if($request->lokasi == 1||$request->lokasi == 3||$request->lokasi == 5){
            ...
            return view('dashboard.report.extends.searchmember',compact('nama_gym','gym','zona','tertentugym','tertentuzona'))->with('gyms',$gyms)
            ->with('currentdate',$currentdate)
            ->with('backdate',$backdate)
            ->with('tofifteen',$tofifteen);
            ...
            return view('dashboard.report.extends.zonamember',compact('nama_gym','gym','zona','tertentugym','tertentuzona'))->with('zonas',$zonas)
            ->with('currentdate',$currentdate)
            ->with('backdate',$backdate)
            ->with('tofifteen',$tofifteen);
        }else{
    ...
    public function link_zonamemberextends(Request $request, $id)
    {
        ...
        $tofifteen = Carbon::parse('15 days')->toDateTimeString();
            ...
            return view('dashboard.report.extends.link_zonamemberbaru',compact('id','nama_gym','gym','zona','tertentugym','tertentuzona'))->with('gyms',$gyms)
            ->with('currentdate',$currentdate)
            ->with('backdate',$backdate)
            ->with('tofifteen',$tofifteen);
    }
    ...
    public function exportExcel(Request $request)
    {
        ...
        return Excel::create('Report Member All', function($excel) use ($members,$gym,$zona){
            $excel->sheet('New sheet', function($sheet) use ($members,$gym,$zona){
                $sheet->loadView('dashboard.report.extends.member_excel')->with('members',$members)
                                                                            ->with('gym',$gym)
                                                                            ->with('zona',$zona);
            });
        })->download('xls');
    ...
    public function exportPDF(Request $request)
    {
        ...
        $pdf = PDF::loadView('dashboard.report.extends.member_excel',compact('members','gym','zona'));
        ...
        if($request->lokasi == 1||$request->lokasi == 3||$request->lokasi == 5||$id!=null)
        {
            return Excel::create('Report Member All', function($excel) use ($gyms,$backdate,$currentdate,$tofifteen){
                $excel->sheet('New sheet', function($sheet) use ($gyms,$backdate,$currentdate,$tofifteen){
                    $sheet->loadView('dashboard.report.extends.search_excel')->with('gyms',$gyms)
                                                                                ->with('currentdate',$currentdate)
                                                                                ->with('backdate',$backdate)
                                                                                ->with('tofifteen',$tofifteen);
            });
            ...
            return Excel::create('Report Member All', function($excel) use ($zonas,$backdate,$currentdate,$tofifteen){
                $excel->sheet('New sheet', function($sheet) use ($zonas,$backdate,$currentdate,$tofifteen){
                    $sheet->loadView('dashboard.report.extends.zona_excel')->with('zonas',$zonas)
                                                                                ->with('currentdate',$currentdate)
                                                                                ->with('backdate',$backdate)
                                                                                ->with('tofifteen',$tofifteen);
            });
    ...
    public function exportPDF1(Request $request)
    {
        ...
        if($request->lokasi == 1||$request->lokasi == 3||$request->lokasi == 5||$id!=null)
        {
            $pdf = PDF::loadView('dashboard.report.extends.search_excel',compact('gyms','backdate','currentdate','tofifteen'));
            return $pdf->download('Report Member All.pdf');
        }else{
            $pdf = PDF::loadView('dashboard.report.extends.zona_excel',compact('zonas','backdate','currentdate','tofifteen'));
            return $pdf->download('Report Member All.pdf');
        }
    }
}
