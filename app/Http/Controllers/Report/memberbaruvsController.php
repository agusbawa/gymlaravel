...
use PDF;
use Excel;
class memberbaruvsController extends Controller
{
    public function member(Request $request)
    {
        ...
        $members = Member::get();
        return view('dashboard.report.memberbaruvs.member',compact('nama_gym','gym','zona','tertentugym','tertentuzona'))->with('members',$members)
    ...
    public function searchmember(Request $request)
    {
        ...
        if($request->lokasi == 1||$request->lokasi == 3||$request->lokasi == 5){
            ...
            return view('dashboard.report.memberbaruvs.searchmember',compact('nama_gym','gym','zona','tertentugym','tertentuzona','tertentugymku'))->with('gyms',$gyms)
            ->with('currentdate',$currentdate)
            ->with('backdate',$backdate)
            ->with('tofifteen',$tofifteen);
            ...
            return view('dashboard.report.memberbaruvs.zonamember',compact('nama_gym','gym','zona','tertentugym','tertentuzona','tertentugymku'))->with('zonas',$zonas)
            ->with('currentdate',$currentdate)
            ->with('backdate',$backdate)
            ->with('tofifteen',$tofifteen);
        }else{
            $members = Member::get();
            return view('dashboard.report.memberbaruvs.member',compact('nama_gym','gym','zona','tertentugym','tertentuzona','tertentugymku'))->with('members',$members)
            ->with('currentdate',$currentdate)
            ->with('backdate',$backdate)
            ->with('tofifteen',$tofifteen);
        }
    ...
    public function link_zonamemberbaru(Request $request, $id)
    {
        ...
        $tofifteen = Carbon::parse('15 days')->toDateTimeString();
            ...
            return view('dashboard.report.memberbaruvs.link_zonamemberbaru',compact('nama_gym','gym','zona','tertentugym','tertentuzona','tertentugymku','id'))->with('gyms',$gyms)
            ->with('currentdate',$currentdate)
            ->with('backdate',$backdate)
            ->with('tofifteen',$tofifteen);
    }
    ...
    {   
        ...
        return Excel::create('Report Member All', function($excel) use ($members,$gym,$zona){
            $excel->sheet('New sheet', function($sheet) use ($members,$gym,$zona){
                $sheet->loadView('dashboard.report.memberbaruvs.member_excel')->with('members',$members)
                                                                            ->with('gym',$gym)
                                                                            ->with('zona',$zona);
            });
        })->download('xls');
    ...
    public function exportPDF(Request $request)
    {
        ...
        $pdf = PDF::loadView('dashboard.report.memberbaruvs.member_excel',compact('members','gym','zona'));
        ...
        if($request->lokasi == 1||$request->lokasi == 3||$request->lokasi == 5||$id!=null)
        {
            return Excel::create('Report Member All', function($excel) use ($gyms,$currentdate,$backdate){
                $excel->sheet('New sheet', function($sheet) use ($gyms,$currentdate,$backdate){
                    $sheet->loadView('dashboard.report.memberbaruvs.search_excel')->with('gyms',$gyms)
                                                                                  ->with('backdate',$backdate)
                                                                                  ->with('currentdate',$currentdate);
                });
            })->download('xls');
            ...
            return Excel::create('Report Member All', function($excel) use ($zonas,$currentdate,$backdate){
                $excel->sheet('New sheet', function($sheet) use ($zonas,$currentdate,$backdate){
                    $sheet->loadView('dashboard.report.memberbaruvs.zona_excel')->with('zonas',$zonas)
                                                                                ->with('backdate',$backdate)
                                                                                ->with('currentdate',$currentdate);
                }); 
            })->download('xls'); 
    ...
    public function exportPDF1(Request $request)
    {
        ...
        if($request->lokasi == 1||$request->lokasi == 3||$request->lokasi == 5||$id!=null)
        {
            $pdf = PDF::loadView('dashboard.report.memberbaruvs.search_excel',compact('gyms','currentdate','backdate'));
                $pdf->setPaper('A4', 'landscape');
                return $pdf->download('Report Member All.pdf');
        }else{
            $pdf = PDF::loadView('dashboard.report.memberbaruvs.zona_excel',compact('zonas','currentdate','backdate'));
                $pdf->setPaper('A4', 'landscape');
                return $pdf->download('Report Member All.pdf');
        }     
    }
