...
use PDF;
use Excel;
class usiamemberbaruController extends Controller
{
    public function member(Request $request)
    {
        ...
        return view('dashboard.report.usiamemberbaru.member',compact('nama_gym','gym','zona','tertentugym','tertentuzona'))
    ...
    public function searchmember(Request $request)
    {
        ...
        ;
            ...
            if($request->lokasi == 1||$request->lokasi == 3||$request->lokasi == 5){
                ...
                return view('dashboard.report.usiamemberbaru.searchmember',compact('nama_gym','gym','zona','tertentugym','tertentuzona'))
                ->with('currentdate',$currentdate)
                ->with('backdate',$backdate)
                ->with('tofiftieen',$tofifteen)
                ->with('gyms',$gyms)
            ;
                ...
                return view('dashboard.report.usiamemberbaru.zonamember',compact('nama_gym','gym','zona','tertentugym','tertentuzona'))
                ->with('currentdate',$currentdate)
                ->with('backdate',$backdate)
                ->with('tofiftieen',$tofifteen)
                ->with('zonas',$zonas)
    ...
    public function link_zonausia(Request $request, $id)
    {
        ...
        $tertentugym = array();
            ...
            $gyms = Gym::orderBy('title','ASC')->where('zona_id',$id)->get();   
                ...
                return view('dashboard.report.usiamemberbaru.link_zonausia',compact('nama_gym','gym','zona','tertentugym','tertentuzona'))
                ->with('currentdate',$currentdate)
                ->with('backdate',$backdate)
                ->with('tofiftieen',$tofifteen)
                ->with('gyms',$gyms)
    ...
    public function exportExcel(Request $request)
    {
        ...
        return Excel::create('Report Member All', function($excel) use ($currentdate,$backdate,$date30DaysBack,$tofifteen){
            $excel->sheet('New sheet', function($sheet) use ($currentdate,$backdate,$date30DaysBack,$tofifteen){
                $sheet->loadView('dashboard.report.usiamemberbaru.member_excel')->with('currentdate',$currentdate)
                                                                            ->with('backdate',$backdate)
    ...
    public function exportPDF(Request $request)
    {
        ...
        $date30DaysBack = Carbon::parse($backdate)->format('Y-m-d');
        $tofifteen = Carbon::parse('15 days')->toDateTimeString();
        $pdf = PDF::loadView('dashboard.report.usiamemberbaru.member_excel',compact('currentdate','backdate','date30DaysBack','tofifteen'));
