...
@section('page-breadcrumb')
    {!!Breadcrumbs::render('packagelist-create')!!}
@endsection
@section('content')
    <div class="row">
        <div class="col-lg-6">
            <form action="/u/packageprices" method="POST">
                <div class="card-box">
                    {{ method_field('POST') }}
                    {{ csrf_field() }}
                    ...
                    <div class="form-group">
                        ...
                        <select name="package_id" id="" class="select2 form-control">
                            @foreach($packages as $package)
                                <option @if($package->id == old('package_id')) selected @endif value="{{$package->id}}">{{$package->title}}</option>
                            @endforeach
                        </select>
                    </div>
