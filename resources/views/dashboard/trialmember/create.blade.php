...
@section('title', 'Tambah Free Trial')
@section('page-title', 'Tambah Free Trial')
@section('page-breadcrumb')
    {!!Breadcrumbs::render('trial-create')!!}
@endsection
@section('content')
    <div class="row">
        <div class="col-lg-6">
            <form action="/u/trial" method="POST">
                <div class="card-box">
                    {{ method_field('POST') }}
                    {{ csrf_field() }}
                    ...
                    <div class="form-group label-floating @if($errors->has('folow_up_by')) has-error @endif">
                        ...
                        <input type="text" class="form-control" name="folow_up_by" value="{{old('folow_up_by')}}">
                    ...
                    <div class="form-group label-floating @if($errors->has('folow_up')) has-error @endif">
                        ...
                        <input type="text" class="form-control datepicker" name="folow_up" value="{{old('folow_up')}}">
                        @if($errors->has('folow_up')) <p class="help-block">{{ $errors->first('folow_up') }}</p> @endif
                    </div>
                    
                    <div class="form-group label-floating @if($errors->has('tanggal_kedatangan')) has-error @endif">
                        <label class="control-label">Tanggal Kedatangan</label>
                        <input type="text" class="form-control datepicker" name="tanggal_kedatangan" value="{{old('tanggal_kedatangan')}}">
                    ...
                    <div class="form-group label-floating @if($errors->has('status')) has-error @endif">
                        ...
                        <select class="form-control select2" name="status">
                            ...
                            @foreach($status as $statusKey => $statusVal)
                                <option @if(old('status')==$statusKey) selected @endif value="{{$statusKey}}">{{$statusVal}}</option>
                            @endforeach
                        </select>
                        @if($errors->has('status')) <p class="help-block">{{ $errors->first('status') }}</p> @endif
                    </div>
                    
                    <div class="form-group label-floating @if($errors->has('remark')) has-error @endif">
