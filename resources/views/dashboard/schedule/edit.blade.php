...
@section('page-title', 'Edit Training Schedule')
@section('page-breadcrumb')
    {!!Breadcrumbs::render('schedule-edit', $train)!!}
@endsection
@section('content')
    <div class="row">
        <form action="/u/schedule/{{$train->id}}" method="POST">
        <div class="col-lg-6">
                <div class="card-box">
                    {{ method_field('PATCH') }}
                    {{ csrf_field() }}
                    <div class="form-group label-floating @if($errors->has('gym_id')) has-error @endif">
                        ...
                        <select class="form-control" id="gym_id" name="gym_id">
                            ...
                            @foreach($gyms as $gym)
                                <option @if(old('gym',$gym->id )== $train->gym_id) selected @endif value="{{$gym->id}}" data-package="{{$gym->package}}">{{$gym->title}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group label-floating @if($errors->has('tgl_training')) has-error @endif">
                        ...
                        <input type="text" class="form-control datepicker" placeholder="Tanggal Training" name="tgl_training" value="{{old('tgl_training',$train->tgl_training)}}">
                    ...
                    <div class="form-group label-floating @if($errors->has('jam')) has-error @endif">
                        ...
                        <input type="text" class="form-control" name="jam" value="{{old('jam',$train->jam)}}">
                    ...
                    <div class="form-group label-floating @if($errors->has('durasi')) has-error @endif">
                        ...
                        <input type="text" class="form-control" name="durasi" value="{{old('durasi',$train->durasi)}}">
                     ...
                     <div class="form-group label-floating @if($errors->has('instruktur')) has-error @endif">
                        ...
                        <input type="text" class="form-control" name="instruktur" value="{{old('instruktur',$train->instruktur)}}">
                    ...
                    <div class="form-group label-floating @if($errors->has('profile_trainer')) has-error @endif">
                        <textarea name="profile_trainer" id="" cols="30" rows="10" class="summernote">{{old('profile_trainer',$train->profile_trainer)}}</textarea>
