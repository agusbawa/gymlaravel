...
@section('help-title', 'Member Perpanjangan Online vs CS')
@section('title', 'Member Perpanjangan Online vs CS')
@section('page-title', 'Member Perpanjangan Online vs CS')
...
<div class="row">
 ...
 <div class="panel panel-default table-responsive" style="text-align:center; overflow:auto;" >
    <table class="table table-bordered text-center" >
        ...
        @foreach($zonas as $gym)
        <tr>
            <td>{{$gym->title}}</td>
            <td>{{$allgym = App\Member::orderBy('members.id','DESC')
                            
                                     ->join('member_histories','member_histories.member_id','=','members.id')
                                     ->join('gyms','gyms.id','=','members.gym_id')->join('zonas','zonas.id','=','gyms.zona_id')->where('zonas.id',$gym->id)     
                                     ->whereBetween('member_histories.extends',[$backdate,$currentdate])
                                     ->get()->count()}}</b></h3>
                                   <div class="persen" style="visibility: hidden;">@if($allgym && $all ){{number_format(($allgym/$all)*100,0,'.','')}}@else 0 @endif%</div></td>
            <td>{{$allgym = App\Member::orderBy('members.id','DESC')
                                    ->where('members.registerfrom','1')
                                     ->join('member_histories','member_histories.member_id','=','members.id')
                                     ->join('gyms','gyms.id','=','members.gym_id')->join('zonas','zonas.id','=','gyms.zona_id')->where('zonas.id',$gym->id)     
...
<script>
 var pieData = [
      @foreach($zonas as $gym)
   {
      ...
      value: @if($allgym = App\Member::orderBy('members.id','DESC')
                            ->where('members.registerfrom','1')
                                     ->join('member_histories','member_histories.member_id','=','members.id')
                                     ->join('gyms','gyms.id','=','members.gym_id')->join('zonas','zonas.id','=','gyms.zona_id')->where('zonas.id',$gym->id)     
                                     ->whereBetween('member_histories.extends',[$backdate,$currentdate])
                                     ->get()->count() && $all)
                                     {{number_format(($allgym/$all)*100,0,'.','')}}
