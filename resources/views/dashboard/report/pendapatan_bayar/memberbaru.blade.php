...
<div class="panel panel-default table-responsive" style="text-align:center; overflow:auto;" >
        <table class="table table-bordered" >
            <tr>
                ...
                <th style="text-align:center">Mesin EDC</th>
                <th style="text-align:center">Transfer</th>
            </tr>
            <tr>
                <td style="text-align:center">Rp {{$total}}</td>
                <td style="text-align:center">Rp {{$satu}}</td>
                <td style="text-align:center">Rp {{$dua}}</td>
                <td style="text-align:center">Rp {{$tiga}}</td>
            </tr>
...
@foreach ($gyms as $gymku)
              ...
              <?php 
                 $paketsatu1 = App\Transaction::where('gym_id',$gymku->id)
                                ->join('transaksi_payments','transactions.id','=','transaksi_payments.transaction_id')
                                ->where('transaksi_payments.payment_method','=','CASH')->get();
                                $paketdua1 = App\Transaction::where('gym_id',$gymku->id)->join('transaksi_payments','transactions.id','=','transaksi_payments.transaction_id')
                                ->where('transaksi_payments.payment_method','=','EDC')->get();
                                $pakettiga1 = App\Transaction::where('gym_id',$gymku->id)->join('transaksi_payments','transactions.id','=','transaksi_payments.transaction_id')
                                ->where('transaksi_payments.payment_method','=','Transfer')->get();
                                    $total1 = 0;
                                    $satu1 = 0;
                                    $dua1 = 0;
                                    $tiga1 = 0;
              ...
              ?>
                 ...
                 <tr>
                <td style="text-align:left">Rp {{$gymku->title}}</td>
                <td>Rp {{$total1}}</td>
                <td>Rp {{$satu1}}</td>
                <td>Rp {{$dua1}}</td>
                <td>Rp {{$tiga1}}</td>
