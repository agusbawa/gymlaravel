...
@section('help-title', 'Member Perpanjangan')
@section('title', 'Member Perpanjangan')
@section('page-title', 'Member Perpanjangan')
...
<div class="row">
            ...
            <div class="col-md-12 text-left" style="margin-bottom: 10px;">
                <form action="/u/report/extendssearch" class="form-inline" method="POST">
                {{ method_field('PATCH') }}
                    {{ csrf_field() }}
                    ...
                    <label class="control-label">Periode</label>
                        <input type="text" class="form-control input-daterange-datepicker" name="range" value="{{old('periode')}}{{Carbon\Carbon::parse($backdate)->format('d-m-Y')}} - {{Carbon\Carbon::parse($currentdate)->format('d-m-Y')}}">
                    </div>
                    ...
                    @else
                        ...
                        <a class="btn btn-sm btn-danger waves-effect waves-light" role="button" href="/exportPDFMemberBaru/search?lokasi={{$nama_gym}}@foreach($tertentugym as $tent)&gyms[]={{$tent}}@endforeach"><i class="glyphicon glyphicon-download-alt"></i> Export PDF</a>
                    @endif
    ...
    <table class="table table-bordered" >
        <tr>
            ...
            <th colspan="5" class="text-center">Member Baru <br/>{{$all = App\Member::orderBy('members.id','DESC')
                                        ...
                                        ->whereBetween('extends',[$backdate,$currentdate])
        ...
        @foreach ($gyms as $gym)
        <tr>
            <td class="text-left">{{$gym->title}}</td>
            <td>{{$allsatu = App\Member::orderBy('members.id','DESC')
                                        ->join('package_prices','package_prices.id','=','members.package_id')
                                        ->join('member_histories','member_histories.member_id','members.id')
                                        ->whereBetween('extends',[$backdate,$currentdate])
                                        ->where('members.gym_id',$gym->id)
                                        ->get()->count()}}
                                        <div class="persen" style="visibility: hidden;">@if($all && $allsatu)
                                            {{number_format(($allsatu/$all)*100,0,',','')}}%
                                            @else
                                            0%
                                            @endif   </div></td>
            <td>{{$satudua =  App\gym::where('gyms.id',$gym->id)
                                    ...
                                    ->where('package_prices.day','=','30')
                                    ->get()->count()
                                     }}<div class="persen" style="visibility: hidden;">@if($paketsatu && $satudua)
                                            {{number_format(($satudua/$paketsatu)*100,0,',','')}}%
                                            @else
                                            0%
                                            @endif   </div></td>
            <td>{{
                                        $satutiga = App\gym::where('gyms.id',$gym->id)
                                        ->join('members','members.gym_id','=','gyms.id')
                                        ->join('member_histories','member_histories.member_id','=','members.id')
                                        ...
                                        ->where('package_prices.day','=','90')
                                        ->get()->count()
                                         }}<div class="persen" style="visibility: hidden;">@if($paketdua && $satutiga)
                                            {{number_format(($satutiga/$paketsatu)*100,0,',','')}}%
                                            @else
                                            0%
                                            @endif   </div></td>
            <td>{{
                                       $satuempat = App\gym::where('gyms.id',$gym->id)
                                        ->join('members','members.gym_id','=','gyms.id')
                                        ->join('member_histories','member_histories.member_id','=','members.id')
                                        ...
                                        ->where('package_prices.day','=','180')
                                        ->get()->count()
                                         }}<div class="persen" style="visibility: hidden;">@if($paketiga && $satuempat)
                                            {{number_format(($satuempat/$paketiga)*100,0,',','')}}%
                                            @else
                                            0%
                                            @endif   </div></td>
       <td> {{
                                        $satulima = App\gym::where('gyms.id',$gym->id)
                                        ->join('members','members.gym_id','=','gyms.id')
                                        ->join('member_histories','member_histories.member_id','=','members.id')
                                        ...
                                        ->where('package_prices.day','=','365')
                                        ->get()->count()
                                         }}<div class="persen" style="visibility: hidden;">@if($paketpat && $satulima)
                                            {{number_format(($satulima/$paketpat)*100,0,',','')}}%
                                            @else
                                            0%
                                            @endif   </div></td>
 ...
 var pieData = [
    @foreach($gyms as $gym)
   {
      ...
      ->get()->count() && $all
                                    ...
                                    ->join('package_prices','member_histories.package_price_id','=','package_prices.id')
                                    ->whereBetween('member_histories.extends',[$backdate,$currentdate])
      ->get()->count()/$all)*100}} @else 0 @endif,
      label: '{{$gym->title}}',
      color: '#811BD6'
   },
   @endforeach
    ...
    [
        {
            ...
            borderWidth: 1,
            data: [@foreach($gyms as $gym)  {{App\gym::where('gyms.id',$gym->id)
                                    ->join('members','members.gym_id','=','gyms.id')
                                    ->join('member_histories','member_histories.member_id','=','members.id')
                                    ->where('members.type','=','New')
                                    ->join('package_prices','member_histories.package_price_id','=','package_prices.id')
