...
<div class="row">
            ...
            <div class="col-md-12 text-left">
               <form action="/u/report/searchusialongvs" class="form-inline" method="POST">
                {{ method_field('PATCH') }}
                    {{ csrf_field() }}
      ...
      <div class="panel panel-default">
        <div class="panel-heading" role="tab" id="headingOne">
          <h4 class="panel-title">
            @if($nama_gym==2) 
                        <a href="/u/report/link_zonausiabaruvs/{{$zona->id}}?range={{$date_awal}}++-++{{$date_akhir}}">
                      @else
        ...
        <div id="{{$zona->id}}" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingOne">
          <div class="panel-body">
            ...
            ->where('members.extended_at','<',$currentdate)
            ->where('members.type','extends')
            ->where('zonas.id',$zona->id)
            ->count()}}</b></h3>
                                        <p class="text-muted"><span id="hidden_persen1{{$zona->id}}" style="visibility: hidden;">0%</span><br>{{$zona->title}}</p>
                                    </div>
                            ...
                            <div class="col-md-6 col-lg-12">
                                <div class="widget-bg-color-icon card-box">
                                    ...
                                    <div class="text-right">
                                        <h3 class="text-dark"><b>{{App\Member::where('members.extended_at','<',$currentdate)
           ->join('member_histories','member_histories.member_id','=','members.id')
            ->join('gyms','gyms.id','=','members.gym_id')
             
            ->where('members.type','=','extends')->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-18))
            ->where('gyms.zona_id','=',$zona->id)
            ->count()}}</b></h3>
                                        <p class="text-muted"><span id="hidden_persen2{{$zona->id}}" style="visibility: hidden;">0%</span><br> > 18 Tahun</p>
                                    </div>
                                    <div class="clearfix"></div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6 col-lg-12">
                                <div class="widget-bg-color-icon card-box">
                                    ...
                                    <div class="text-right">
                                        ...
                                        {{App\Member::where('members.extended_at','<',$currentdate)
           ->join('member_histories','member_histories.member_id','=','members.id')
            ->join('gyms','gyms.id','=','members.gym_id')
             
            ->where('members.type','=','exte')->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-18))->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-24))
            ->where('gyms.zona_id','=',$zona->id)
            ->count()}}
            </b></h3>
           ...
           ->join('member_histories','member_histories.member_id','=','members.id')
            ...
            ->where('members.type','=','extends')->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-18))->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-24))
            ->where('gyms.zona_id','=',$zona->id)
            ->count()}}</b></h3>
                                        <p class="text-muted">Online</p>
                                    </div>
           ...
           ->join('member_histories','member_histories.member_id','=','members.id')
            ...
            ->where('members.type','=','extends')->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-25))->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-34))
            ->where('gyms.zona_id','=',$zona->id)
            ->count()}}</b></h3>
           ...
           ->join('member_histories','member_histories.member_id','=','members.id')
            ...
            ->where('members.type','=','extends')->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-35))->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-44))
            ->where('gyms.zona_id','=',$zona->id)
            ->count()}}</b></h3>
           ...
           ->join('member_histories','member_histories.member_id','=','members.id')
            ...
            ->where('members.type','=','extends')->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-45))->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-54))
            ->where('gyms.zona_id','=',$zona->id)
            ->count()}}</b></h3>
           ...
           ->join('member_histories','member_histories.member_id','=','members.id')
            ...
            ->where('members.type','=','extends')->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-55))->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-64))
            ->where('gyms.zona_id','=',$zona->id)
            ->count()}}</b></h3>
           ...
           ->join('member_histories','member_histories.member_id','=','members.id')
            ...
            ->where('members.type','=','extends')->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-65))
            ->where('gyms.zona_id','=',$zona->id)
            ->count()}}</b></h3>
