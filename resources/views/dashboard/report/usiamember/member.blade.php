...
<div class="row">
            ...
            <div class="col-md-12 text-left">
                ...
                {{ method_field('PATCH') }}
                    ...
                    <a class="btn btn-sm btn-success waves-effect waves-light" role="button" href="/exportExcelusiamember"><i class="glyphicon glyphicon-download-alt"></i> Export Excel</a>
                    <a class="btn btn-sm btn-danger waves-effect waves-light" role="button" href="/exportPDFusiamember"><i class="glyphicon glyphicon-download-alt"></i> Export PDF</a>
                        ...
                        <table class="table table-bordered" >
                            ...
                            <tr>
                                <td>{{$all=App\Member::join('member_histories','member_histories.member_id','members.id')
                                 ...
                                 ->orWhereBetween('member_histories.extends',[$backdate,$currentdate])
                                ->get()->count()}}
...
<script>
 var pieData = [
   {
      value: @if($eight && $all){{number_format(($eight/$all)*100,0,'.','')}}@else 0 @endif,
      label: '< 18',
      color: '#811BD6'
   },
   {
      value: @if($twenty && $all){{number_format(($twenty/$all)*100,0,'.','')}}@else 0 @endif,
      label: '18 - 24',
      color: '#8C9BD6'
   },
   {
      value: @if($thirty && $all){{number_format(($thirty/$all)*100,0,'.','')}}@else 0 @endif,
      label: '25 - 24',
      color: '#857886'
   },
   {
      value: @if($fourty && $all){{number_format(($fourty/$all)*100,0,'.','')}}@else 0 @endif,
      label: '35 - 44',
      color: '#857886'
   },
    {
      value: @if($fifty && $all){{number_format(($fifty/$all)*100,0,'.','')}}@else 0 @endif,
      label: '45 - 54',
      color: '#857a77'
   },
   {
      value: @if($sixty && $all){{number_format(($sixty/$all)*100,0,'.','')}}@else 0 @endif,
      label: '55 - 64',
      color: '#857ac5'
   },
   {
      value: @if($seventy && $all){{number_format(($seventy/$all)*100,0,'.','')}}@else 0 @endif,
      label: '65+',
      color: '#857ac5'
   },
