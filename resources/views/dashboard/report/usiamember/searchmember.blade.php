...
<div class="panel panel-default table-responsive" style="text-align:center; overflow:auto;" >
                        <table class="table table-bordered" >
                            ...
                            <tr>
                                <td>{{$gym->title}}</td>
                                <td>{{$sem = $gym->members()->get()->count()}}</td>
                                <td>{{$barulap = $gym->members()->orderBy('nick_name','asc')->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-18))->get()->count()}}</b></h3>
                                <p class="text-muted"><span class="persen" {{$gym->id}}" style="visibility: hidden;">@if($barulap && $eight){{number_format(($barulap/$eight)*100,0,'.','')}} @else 0 @endif %</span></td>
                                <td>{{$barupat = $gym->members()->orderBy('nick_name','asc')->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-18))->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-24))->get()->count()}}</b></h3>
                                ...
                                <td>{{$barulima = $gym->members()->orderBy('nick_name','asc')->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-25))->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-34))->get()->count()}}</b></h3>
                                <p class="text-muted"><span class="persen" style="visibility: hidden;">@if($barulima && $thirty){{number_format(($barulima/$thirty)*100,0,'.','')}} @else 0 @endif%</span></td>
                                <td>{{$barunam = $gym->members()->orderBy('nick_name','asc')->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-35))->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-44))->get()->count()}}</b></h3>
                                <p class="text-muted"><span class="persen" style="visibility: hidden;">@if($barunam && $fourty){{number_format(($barulima/$thirty)*100,0,'.','')}} @else 0 @endif%</span></td>
                                <td>{{$barujuh = $gym->members()->orderBy('nick_name','asc')->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-45))->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-54))->get()->count()}}</b></h3>
                                <p class="text-muted"><span class="persen" style="visibility: hidden;">@if($barulima && $fifty){{number_format(($barujuh/$fifty)*100,0,'.','')}} @else 0 @endif%</span></td>
                                <td>{{$barupan = $gym->members()->orderBy('nick_name','asc')->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-55))->where('date_of_birth','>',Carbon\Carbon::now()->addYear(-64))->get()->count()}}</b></h3>
                                ...
                                <td>{{$barulan = $gym->members()->orderBy('nick_name','asc')->where('date_of_birth','<',Carbon\Carbon::now()->addYear(-65))->get()->count()}}</b></h3>
...
<script>
 ...
 var pieData = [
     @foreach($gyms as $gym)
   {
      value: @if( $gym->members()->get()->count() &&  App\Member::get()->count()){{number_format(($gym->members()->get()->count() /App\Member::get()->count())*100,0,'.','')}} @else 0 @endif,
      label: "{{$gym->title}}",
      color: '#811BD6'
   },
   @endforeach
